...
In Microsoft Dynamics 365 Finance version 10.0.12 (August 2020), you can use company ranges as part of the condition for data selection in [FILTER](er-functions-list-filter.md)-based data sources that are called within the scope of execution of a **JOIN** data source. Because of the limitations of the application [query (This is an external linThis link was changed due to HTMLfromRepoGenerator)](../dev-ref/xpp-library-objects.md#query (This is an external linThis link was changed due to HTMLfromRepoGenerator)-object-model) builder, the company ranges are supported only for the first data source of a **JOIN** data source.
